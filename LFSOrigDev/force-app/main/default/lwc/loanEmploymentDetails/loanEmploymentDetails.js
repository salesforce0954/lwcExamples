...
export default class LoanEmploymentDetails extends LightningElement {
    ...
    @api employmentDetailsObj;
    @api isJointApplication;
    @api inputMode;
    @track copyEmploymentDetailsObj;
    incomeObj = {};
    ...
    connectedCallback() {
        this.copyEmploymentDetailsObj = { ...this.employmentDetailsObj };
        if (this.copyEmploymentDetailsObj.IncomeInfo !== undefined && this.copyEmploymentDetailsObj.IncomeInfo.length > 0) {
            this.incomeDetailsList = [...this.copyEmploymentDetailsObj.IncomeInfo];
        }
   ...
   */
    ...
    handleAddIncome() {
        ...
        if (inputPicklistCorrect) {
            ...
            if (this.incomeDetailsList.length === 0) {
                let incomeQuote = this.copyEmploymentDetailsObj.IncomeQuote;;
                this.incomeObj.IncomeFrom = incomeQuote;
                if (this.copyEmploymentDetailsObj.OccupationQuote !== undefined) {
                    this.incomeObj.Occupation = this.copyEmploymentDetailsObj.OccupationQuote;
                }
                this.incomeObj.EmploymentYears = this.copyEmploymentDetailsObj.EmploymentYearsQuote;
                this.incomeObj.EmploymentMonths = this.copyEmploymentDetailsObj.EmploymentMonthsQuote;
    ...
    handleAddIncomeSave() {
        //this.copyEmploymentDetailsObj.LoanInfo.push(this.loanObj);
    ...
    handleEmploymentDetailsChange(event) {
        ...
        if (fieldName === 'employmentStatus') {
            this.copyEmploymentDetailsObj.EmploymentStatus = fieldValue;
        } else if (fieldName === 'previousEmploymentStatus') {
            this.copyEmploymentDetailsObj.PreviousEmploymentStatus = fieldValue;
        } else if (fieldName === 'prevEmpYears') {
            this.copyEmploymentDetailsObj.PrevEmploymentYears = fieldValue;
        } else if (fieldName === 'prevEmpMonths') {
            this.copyEmploymentDetailsObj.PrevEmploymentMonths = fieldValue;
        }
    ...
    handleEmploymentDetailsNext() {
        ...
        if (this.handleValidationOnNext()) {
            this.copyEmploymentDetailsObj.IncomeInfo = this.incomeDetailsList;
    ...
    fireEmploymentDetailsNotifyEvent() {
        this.dispatchEvent(new CustomEvent('employmentdetailsinfo', {
            detail: {
                completedStep: 'step-3',
                nextStep: 'step-4',
                employmentDetails: this.copyEmploymentDetailsObj
            }
        }));
    }
    ...
    handleGoToJointApplicant() {
        if (this.handleValidationOnNext()) {
            ...
            this.dispatchEvent(new CustomEvent('navigatejointemployment'));
        }
    }
